...
const TxModal = ({
  ...
  const queryFinder = (params: string) => {
    const network =
      `${wallet?.network.chainID}` === "bombay-12" ? "testnet" : "mainnet";
    const type_of_find = params
      ? params.length === 64
        ? "tx"
        : "address"
      : "";
  ...
  const Dialog = ({ type, title, img }: any) => {
    return (
      <div
        className={st_tx.dialog}
        style={{
          padding:
            status === TxStep.Success || status === TxStep.Failed
              ? "25px 50px"
              : "50px",
        }}
      >
        ...
        <div className={st_tx.title_tx}>
          ...
          {type !== TxStep.Waiting && type !== TxStep.Failed && (
            <h3>
              Tx Hash:{" "}
              <span>
                <a href={queryFinder(txHash)} target="_blank">
    ...
    <Modal isCentered isOpen={open} onClose={() => null} size="sm">
      ...
      <ModalContent>
        {status === TxStep.Waiting && (
          <Dialog
            img="images/png/angelwing_bl.png"
